# Smart Contract Study 14.07.2017
...
data <- data %>% 
  rename(category = data.CATEGORY, txs = data.sumTXs)
...
distribution <- data.frame( 
  names = c("Financial", "Notary", "Wallet", "Game", "Library"),
  value = c(Financial, Notary, Wallet, Game, Library),
...
usagePlot <- ggplot(data=distribution, aes(distribution$names, distribution$percentage,  fill = "red")) +  
  geom_bar(stat = "identity", show.legend = FALSE) +
  ggtitle("Usage of Ethereum Smart Contracts by Categories") + 
  xlab("Categories") +
  ylab("Percentage") +
