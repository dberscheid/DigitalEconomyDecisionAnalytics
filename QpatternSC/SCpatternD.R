...
dataPat     = dataCompl %>% group_by(PATTERNS) %>% mutate(sumTXspP = sum(TXs))
...
patSummary  = patSummary %>% rename(category = patSummary.CATEGORY, pattern = patSummary.PATTERNS, txs = patSummary.sumTXspP)
...
#looping over all patterns and all categories
...
final = data.frame(category = c("Financial", "Notary", "Game", "Wallet", "Library"), token, authorization, oracle, poll, termination, math)
